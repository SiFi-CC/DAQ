...
class Connection:
	...
	def __set_test_pulse(self, targets, length, interval, finePhase, invert=False):
		...
		for tacRefreshPeriod_1, tacRefreshPeriod_2 in self.__asicConfigCache_TAC_Refresh:
			...
			if interval % tacRefreshPeriod == 0:
				logging.info("WARNING: Test pulse period %d is a multiple of TAC refresh period %d (%d %d) in some ASICs." % (interval, tacRefreshPeriod, tacRefreshPeriod_1, tacRefreshPeriod_2))
	...
	def initializeSystem(self, maxTries = 5):
		...
		portsStr = (", ").join([str(x) for x in self.getActivePorts()])
		logging.info(f"active units on ports: {portsStr}")
		...
		if self.getTriggerUnit() is not None:
			...
			portID, slaveID = self.getTriggerUnit()
			logging.info("INFO: TGR unit is (%2d, %2d)" % (portID, slaveID))
		...
		if inconsistentStateAsics != []:
			...
			for portID, slaveID in self.getActiveFEBDs():
				...
				if lst != []:
					logging.info(" FEB/D (%2d, %2d)" % (portID, slaveID))
					for chipID, statusTripplet in lst:
						a, b, c = statusTripplet
						a = a and "OK" or "FAIL"
						b = b and "OK" or "FAIL"
						c = c and "OK" or "FAIL"						
						logging.info("  ASIC %2d: config link: %4s data link pattern: %4s data word: %4s" % (chipID, a, b, c))
		...
		for portID, slaveID in self.getActiveFEBDs():
			...
			if lst != []:
				...
				logging.info("INFO: FEB/D (%2d, %2d) has %2d active ASICs: %s" % (portID, slaveID, n, lst))
			else:
				logging.info("INFO: FEB/D (%2d, %2d) has 0 active ASICs." % (portID, slaveID))
	...
	def sendCommand(self, portID, slaveID, cfgFunctionID, payload, maxTries=10):
		...
		while doOnce or (reply == None and nTries < maxTries):
			...
			nTries = nTries + 1
			if nTries > 5: logging.info("Timeout sending command. Retry %d of %d" % (nTries, maxTries))
	...
	def ___doAsicCommand(self, portID, slaveID, chipID, command, value=None, channel=None):
		...
		if isRead:
			...
			if len(reply) < (1+expectedBytes): 
				logging.info(len(reply), (1+expectedBytes))
	...
	def acquire(self, acquisitionTime, step1, step2):
		...
		while currentFrame < stopFrame:
			...
			if (currentFrame - lastUpdateFrame) * frameLength > 0.1:
				...
				lastUpdateFrame = currentFrame
		t1 = time()
		logging.info("Python:: Acquired %d frames in %4.1f seconds, corresponding to %4.1f seconds of data (delay = %4.1f)" % (nFrames, time()-t0, nFrames * frameLength, (t1-t0) - nFrames * frameLength))
		...
		for portID, slaveID in self.getActiveFEBDs():
			...
			for n in range(64):
				if (asic_bad_rx & (1 << n)) != 0:
					...
					logging.info("ASIC (%2d, %2d, %2d) RX links are down (0b%d%d)" % (portID, slaveID, n, b, a))
					bad_rx_found = True
